...
public class UI extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        // Position window on the far right side of the screen
        ...
        // Initialize UI
        if (initialRun == 1) {
            Task<String> task = mic.runPythonScript(pythonExe, 0,
                    initialRun, labelMic, textBox, button);
            ...
            // Start the background thread for the task
        ...
        scene.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.ESCAPE) {
                primaryStage.close(); // Pressing ESC closes the program
                Platform.exit();
                System.exit(0);
            }
